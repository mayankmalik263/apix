"""
AYUSH — your verification tool. Round 3.
...
WHY YOU NEED IT
    Your job is to work out the index by hand and check whether Bharat's code
    got the same answer. If you both use the same code, you have checked
    nothing. Using a spreadsheet makes your check genuinely independent -- and
    "we recomputed it by hand and it matched" is a sentence a jury believes.

RUN IT
    python scripts/dump_for_ayush.py DEL-BOM 2026-09-04
...
def main() -> None:
    if len(sys.argv) != 3:
        print(__doc__)
        print("\nUsage:  python scripts/dump_for_ayush.py <ROUTE> <DATE>")
        print("Example: python scripts/dump_for_ayush.py DEL-BOM 2026-09-04")
        sys.exit(1)
    ...
    print("build plan under 'Ayush, Round 3'.")
